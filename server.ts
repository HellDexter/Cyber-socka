...
// Disable strict SSL certificate checks to prevent "fetch failed" / certificate errors on Czech sites
...
Všechny texty musí být vygenerované tak, aby po zkopírování vypadaly skvěle a měly správné formátování. Odpověď musíte vrátit ve formátu JSON podle zadaného schématu.`,
  ...
  async function scrapeUrl(url: string): Promise<string> {
    ...
    // Set a strict 4-second timeout to prevent Vercel serverless function timeouts
    ...
    try {
      ...
      return text.substring(0, 10000); // return top 10k chars
    } catch (fetchErr: any) {
      clearTimeout(timeoutId);
      if (fetchErr.name === "AbortError") {
        throw new Error("Stažení stránky vypršelo (limit 4 sekundy). Webová stránka neodpovídá dostatečně rychle.");
      }
      throw fetchErr;
    }
  }
